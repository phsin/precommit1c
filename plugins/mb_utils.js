...
function replaceLineNum(doc) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        var selStartCol = doc.SelStartCol;
        var selEndCol = doc.SelEndCol;
        lines[i] = lines[i].replace(re, "НомерСтрокиМодуля("+(i+1)+")");
    ...
    //      corrections++;
    //  }
        if ( lines[i].indexOf( "НомерСтрокиМодуля(" )>-1 ){
            corrections++;
            bChangeSel = 1;
            Message(" #" + lines[i] );
            doc.range(i, 0, i, lines[i].length) = lines[i];
            doc.MoveCaret(doc.SelStartLine, selStartCol, doc.SelEndLine, selEndCol);
            //doc.range(doc.SelStartLine,doc.SelStartCol, doc.SelEndLine, doc.SelEndCol) = Join(lines[i], vbCrLf);
        }
    }
//    if (bChangeSel) {
//        doc.MoveCaret(doc.SelStartLine, selStartCol, doc.SelEndLine, selEndCol);
//    }
    if (corrections)
        Message("Колво замен НомерСтрокиМодуля : " + corrections);
}
